...
#### Передача сообщения и маршруты
##### Маршрут
Для сохранения анонимности пользователей передача сообщений, как уже сказано выше, будет проходить через нескольких других пользователей. Чтобы никто из этой цепочки не мог понять кто автор и получатель сообщения, эта цепь (или же маршрут) будет поочередно зашифрована. Получается схема похожая на TOR, однако вместо всего сообщения шифруется лишь инструкция о том куда передавать сообщение. Аналогию можно провести следующую:
Вместо того чтобы просто засунуть записку в лук, вы саму записку просто заворачивайте в конверт и с конвертом даете лук, на каждой кожурке которого указан человек которому надо передать записку и остаток лука.
Таким образом сам весь маршрут остается тайной для обоих сторон.
##### Отправка сообщения
В криптографии принято называть отправителя Алисой, получателя Бобом, а злоумышленика Евой. Далее будут использоваться эти именна.
* Алиса хочет отправить сообщение "привет" бобу. Изначально Алиса не знает даже публичного ключа боба, так как это первое сообщение в их переписке.
* Алиса перебирает блоки в третьем блокчейне и находит там публичный ключ Боба.
* Для удобства поиска Алиса берет хэш от публичного ключа Боба и начинает искать его маршрут в первом блокчейне.
* Теперь Алиса знает куда нужно отправить сообщение с ключом шифрования чтобы начать переписку с Бобом. Но Алиса не знает IP адрес Боба, так что зная топологию всей сети она дополняет маршрут Боба так чтобы получился маршрут где первая нода это её сосед.
* Теперь она может зашифровать публичным ключом Боба их сессионый, симметричный ключ.
* После того как Боб получит этот ключ он подтвердит Алисе что разговор начат.
* Алиса после подтверждения начала разговора шифрует своё сообщение "привет" их сессионым ключом и отправляет Бобу. Цель выполнена

Таким образом Ева, в свою очередь, не может просто так подделать публичный ключ Боба, так как он уже подписан следующим блоком в третьем блокчейне. Максимум Ева может остановить/задержать отправку сообщения если она является узлом. Но Ева может это делать только в слепую, так как не видит от кого и кому идет сообщение.

### Заключение
В этой статье описывается базовый принцип работы протокола, позволяющего максимально безопасно передавать информацию между людьми. В ближайшее время будет создана первая испытательная версия
